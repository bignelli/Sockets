...
public class SocketRemoverBlock extends Block {
    ...
    public ActionResultType onBlockActivated(BlockState state, World worldIn, BlockPos pos, PlayerEntity player, Hand handIn, BlockRayTraceResult p_225533_6_) {
        if (!worldIn.isRemote()) {
            ...
            if (tile instanceof SocketRemoverTileEntity)
                NetworkHooks.openGui((ServerPlayerEntity) player, (SocketRemoverTileEntity) tile, pos);
    ...
    public void onReplaced(BlockState state, World worldIn, BlockPos pos, BlockState newState, boolean isMoving) {
        if (newState.getBlock() != state.getBlock()) {
            ...
            if (te instanceof SocketRemoverTileEntity) {
                te.getCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY).ifPresent(h -> {
                    final int slots = h.getSlots();
                    for (int i = 0; i < slots; i++) {
                        final ItemStack stack = h.getStackInSlot(i);
                        if (!stack.isEmpty()) InventoryHelper.spawnItemStack(worldIn, pos.getX(), pos.getY(), pos.getZ(), stack);
                    }
                });
            }
        }
